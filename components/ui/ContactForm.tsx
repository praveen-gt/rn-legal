...
export function ContactForm() {
  ...
  const onSubmit = async (data: ContactFormData) => {
    ...
    try {
      ...
      if (!res.ok) {
        setServerError(json.error ?? 'Something went wrong. Please call us directly.')
        setStatus('error')
        return
      }
      ...
      setServerError('Unable to send your enquiry. Please call us directly on 0470 695 941.')
      setStatus('error')
    }
  }
  ...
  if (status === 'success') {
    return (
      <div className="text-center py-16">
        ...
        <h3 className="font-display font-semibold text-white text-2xl mb-3">Enquiry Received</h3>
        <p className="text-white/65 font-body text-sm leading-relaxed max-w-sm mx-auto mb-2">
          Thank you. We will be in touch within 1 business day.
        </p>
        <p className="text-white/50 font-body text-xs leading-relaxed max-w-sm mx-auto mb-6">
          A confirmation has been sent to your email address. For urgent matters, please call us directly.
        </p>
        <button onClick={() => setStatus('idle')} className="btn-outline !text-xs">
          Submit Another Enquiry
        </button>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-8">
      ...
      {status === 'error' && serverError && (
        <div className="flex items-start gap-3 border border-red-400/30 px-5 py-4"
          style={{ background: 'rgba(248,113,113,0.06)' }}>
          <AlertCircle size={15} className="text-red-400 mt-0.5 shrink-0" />
          <p className="text-red-300 font-body text-sm leading-relaxed">{serverError}</p>
        </div>
      )}
        ...
        <label className="flex items-start gap-4">
          <div className="relative mt-0.5">
            <input {...register('agreeToTerms')} type="checkbox"
              className="peer sr-only" />
            <div className="w-4 h-4 border border-white/30 peer-checked:border-gold-400 peer-checked:bg-gold-400/10 transition-all duration-200 flex items-center justify-center">
              <div className="w-2 h-2 bg-gold-400 opacity-0 peer-checked:opacity-100 transition-opacity"
